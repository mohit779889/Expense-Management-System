...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const config = {
      ...
      // Additional Axios configurations can be specified here
    };
    ...
    try {
      ...
      if (data.success) {
        ...
        // Log this case which shouldn't normally happen if the API is well-designed
        console.error('Registration unsuccessful with message:', data.message);
        toast.error(data.message, toastOptions);
      }
    } catch (error) {
      // Handle errors from the request itself, like network issues, or problems reading the response
      ...
      setLoading(false); // Stop loading indication regardless of outcome
    }
  };
            ...
            <Col xs={12} sm={10} md={8} lg={6} className="mx-auto ">
              <div className="form-container">
                ...
                <Form>
                  ...
                  <div style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }} className="mt-3">
                    ...
                    <p style={{ color: "#9d9494" }}>Already have an account? <Link to="/login" className="text-white" >Login</Link></p>
                  </div>
                </Form>
              </div>
            </Col>
